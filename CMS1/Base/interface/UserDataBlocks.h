...
namespace cms1 {
  ...
  template <class T> struct VectorUserBlock: public BaseUserBlock
  {
    ...
    template <class Z> void addCollections( const Z data ) {
      streamer.addCollections(data);
    }

    template <class Z> void addCollections( const Z data1, const Z data2) {
      streamer.addCollections(data1, data2);
    }
