...
def printDict(dict,description):
    ...
    sorted.sort()
    print '-----------------------------------------------------------------------------------------'
    print '| %20s | Processing | Merge      | Cleanup    | LogCollect | Total      |' % description
    print '-----------------------------------------------------------------------------------------'
    total_processing = 0
    total_merge = 0
    total_cleanup = 0
    ...
    for site in sorted:
        ...
        print '| %20s | %10d | %10d | %10d | %10d | %10d |' % (site,dict[site]['Processing'],dict[site]['Merge'],dict[site]['Cleanup'],dict[site]['LogCollect'],dict[site]['Processing']+dict[site]['Merge']+dict[site]['Cleanup']+dict[site]['LogCollect'])
    print '-----------------------------------------------------------------------------------------'
    print '| %20s | %10d | %10d | %10d | %10d | %10d |' % ('Total',total_processing,total_merge,total_cleanup,total_logcollect,total)
    print '-----------------------------------------------------------------------------------------'
...
for line in out.split('\n') :
    ...
    if len(array) > 6: removereason = "DEFINED"    
    type = ''
