...
for run in sorted_runs_one:
    if run in sorted_runs_two:
        ...
        if run not in fileOneRunInNone:
            fileOneRunInNone.append(run)
...
for run in sorted_runs_two:
    if run in sorted_runs_one:
        ...
        if run not in fileTwoRunInNone:
            fileTwoRunInNone.append(run)

fileOneRunInNone.sort()
fileTwoRunInNone.sort()
            
if len(fileOneRunInNone) > 0 :
    print ''
    print 'Following runs are not in file one:'
    print ','.join(fileOneRunInNone)
    print ''
elif len(fileTwoRunInNone) > 0 :
    print ''
    print 'Following runs are not in file two:'
    print ','.join(fileTwoRunInNone)
    print ''
else :
    print ''
